...
func (transfer *Transfer) ReadPkg() (mes message.Message, err error) {
	...
	if err != nil {
		fmt.Println("readPkg json Unmarshal error:", err)
		return
	}
...
func (transfer *Transfer) WritePkg(data []byte) (err error) {
	...
	if n != 4 || err != nil {
		fmt.Println("writePkg conn.Write buf error: ", err)
		return
	}
	...
	if n != int(pkgLen) || err != nil {
		fmt.Println("writePkg conn.Write data error: ", err)
		return
	}
	return
}
